...
#define STREAM_NOT_NULL_ELSE_FAILED(stream) \
if ((stream) == NULL) \
{ \
	return FLW_ST.failed; \
}

uint8_t* get_current_stream_addres(stream_ptr_t stream)
{
	return
...
get_stream_size(
	...
	(*psize) = (*stream).metha.capacity - (*stream).metha.available;
	return FLW_ST.succeeded;
}
